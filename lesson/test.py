    ...
    for i in range(st.session_state["n_rounds"]):
        ...
        if not bot_response:
            ...
            st.session_state["history"].append(
                TextMsg({"role": "assistant" if current_role == "user" else "user", "content": bot_response}))
            with st.chat_message(name=current_role, avatar=current_role):
                st.markdown(query)
            with st.chat_message(name="assistant" if current_role == "user" else "user",
                                 avatar="assistant" if current_role == "user" else "user"):
